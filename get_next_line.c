...
Алгоритм:
1. Принять файл и прочитать его в переменную buff;
2. Создать статическую переменную storage, которая содержит все строки (именно статическую
т.к. она будет изменяться в процессе исполнения всех функций, а не только одной);
3. В переменную line передать первую строку до символа новой строки '\n' из переменной
storage. После того как передал - взять ссылку с начала следующей строки;
4. Отправить в main обратно заполненную line переменную;
5. Сохранить ссылку на следующую строку и fd в листах, чтобы в следующем n-oм вызове отправить
n-ую строку файла.
