...
static int			gnl_verify_line(char **stack, char **line)
{
	...
	//5. И вот если встретил символ след строки (стр-ка 71), то берем адрес этого индекса и присваиваем туда конец строки.
	*tmp_stack = '\0';
	*line = ft_strdup(*stack);//6.отправляем на ft_strdup, чтобы выделилась память из оперативки и 
	//скопировалась линию до символа КОНЦА СТРОКИ.. 
...
/*while*/ if ((ret = read(fd, heap, BUFF_SIZE)) > 0) 
		//1.Читаем дескриптор в heap, который размером с BUFF_SIZE
		//Кол-во байтов присваиваем ret;
		//;
	{
		heap[ret] = '\0'; 
		...
		{
			tmp_stack = *stack; 
			//4.присвоили содержание буфера
			//который содержит указатель уже не на первую, а n-ю линию
			*stack = ft_strjoin(tmp_stack, heap);
			//5.ДЛЯ ЧЕГО?
			//Объединяем их и получаем новую строку;
			free(tmp_stack);
			//6.очищаем временный стэк;
			tmp_stack = NULL;
			//7.и для безопасности обнуляем;
		}
		else
			*stack = ft_strdup(heap);
		//а если стэк не пришёл с содержимым, то кучу с размером в буфе
		//отправляем для созд новой строки за счёт опер памяти и 
		//присваиваем стэку содержание heap;
		if (gnl_verify_line(s	tack, line)) //отпр-ем в функцию, которая ищет симв 
...
int					get_next_line(int const fd, char **line)
{
	...
	if (stack[fd]) //2.обращаемся к нашему файлу
		if (gnl_verify_line(&stack[fd], line)) //3.определяем линию, 
			//отправив аргументами наш стэк под номером файлового деcкр-ра
	...
	while (i < BUFF_SIZE) 
		heap[i++] = '\0';
	//4.Мы выделяли память в дейст 1  и теперь его заполняем просто нулями. 
